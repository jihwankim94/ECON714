...
//Monetary policy shock is not in period t information set of agents 
...
//in the 'R' economy, all exogenous variables are set to their steady states, except the monetary shock
//in the 'F' economy, the monetary shock is set to zero while all other shocks are active
//in the 'AGG' economy, 'R' and 'F' economies are aggregated
...
//set exogenous variables to steady states 
#epsilonR=epsilon_SS;
#upsilonR=upsilon_SS;
...
var muz_eps         = 1; //neutral technology
var mupsi_eps       = 1; //investment technology
end;    
...
sigmaL,,,,gamma_pdf,1,0.25; %EHL model only
...
model_resp_mon_baseline=model_resp_mon;
model_resp_ntech_baseline=model_resp_ntech;
model_resp_itech_baseline=model_resp_itech; 
plot_var_model_irf; 
