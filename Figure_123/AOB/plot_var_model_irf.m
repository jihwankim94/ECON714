...
for plotindx=1:1:size(policyresponse,2);
    ...
    plot(time,model_resp_itech_baseline(reorderidx(plotindx),1:pnstep),'b-'); hold on
    
    if plotindx==1,
            legend_string={['VAR ',num2str(conflevel*100),'%'],'VAR Mean',' Alternating Offer Bargaining (AOB)'}; 
    end
    hold off
...
for plotindx=1:1:size(policyresponse,2);
    ...
    plot(time,model_resp_ntech_baseline(reorderidx(plotindx),1:pnstep),'b-'); hold on
...
for plotindx=1:1:size(policyresponse,2);
    ...
    plot(time,model_resp_mon_baseline(reorderidx(plotindx),1:pnstep),'b-'); hold on
